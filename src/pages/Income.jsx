                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left min-w-[600px]">
                                <thead className="border-b border-gray-50 dark:border-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                                    <tr>
                                        <th className="px-6 md:px-8 py-6">Date</th>
                                        <th className="px-6 md:px-8 py-6">Source</th>
                                        <th className="px-6 md:px-8 py-6">Description</th>
                                        <th className="px-6 md:px-8 py-6 text-right">Amount</th>
                                ...
                                <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                                    {filteredIncomes.map((income) => (
                                        <tr key={income.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                                            <td className="px-6 md:px-8 py-6 text-xs md:text-sm text-gray-500">
                                            ...
                                            <td className="px-6 md:px-8 py-6 text-xs md:text-sm font-bold text-gray-900 dark:text-white uppercase tracking-tight">
                                                {income.description || 'Generic Drop'}
                                            </td>
                                            <td className="px-6 md:px-8 py-6 text-right">
                                                <span className="text-base md:text-lg font-black text-emerald-600 dark:text-emerald-500">
                                                    {formatCurrency(income.amount)}
                                                </span>
                                            </td>
                                            <td className="px-6 md:px-8 py-6">
                                                <div className="flex justify-end gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => { setEditingIncome(income); setIsFormOpen(true); }} className="p-2 text-gray-400 hover:text-blue-500">
                                                        <Edit2 size={16} />
                                                    </button>
                                                    <button onClick={() => handleDelete(income.id)} className="p-2 text-gray-400 hover:text-red-500">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            </td>
